...
void initBluetooth() {
  ...
  // Set I2S configuration for Bluetooth
...
void volumeUp() { 
  ...
  audio.setVolume(currentVolume);
  a2dp_sink.setVolume(currentVolume * 6);  // 0-128
  updateDisplay(); 
}
void volumeDown() { 
  currentVolume = max(0, currentVolume-VOL_STEP); 
  audio.setVolume(currentVolume);
  a2dp_sink.setVolume(currentVolume * 6);
  updateDisplay(); 
}
...
void updateDisplay() {
  ...
  // Mode content (full screen usage)
  tft.setTextSize(2);
...
void setupWebServer() {
  ...
  server.on("/api/wifi/scan", HTTP_GET, []() {
    ...
    for (int i = 0; i < wifiNetworkCount; i++) {
      j += "{\"ssid\":\"" + wifiNetworks[i].ssid + "\",\"rssi\":" + String(wifiNetworks[i].rssi);
      j += ",\"enc\":" + String(wifiNetworks[i].encType != WIFI_AUTH_OPEN ? 1 : 0) + "}";
      if (i < wifiNetworkCount-1) j += ",";
    }
    j += "]}";
    server.send(200, "application/json", j);
  });
  ...
  server.on("/api/wifi/connect", HTTP_POST, []() {
    ...
    int ss = body.indexOf("\"ssid\":\"")+8, se = body.indexOf("\"", ss);
    String ssid = body.substring(ss, se);
    int ps = body.indexOf("\"password\":\"")+12, pe = body.indexOf("\"", ps);
    String pass = body.substring(ps, pe);
    
    server.send(200, "application/json", "{\"message\":\"Connecting...\"}");
    delay(100);
