/*
 * NetRadio v5.5 - БЕЗ BLUETOOTH (для тестирования)
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n=== NetRadio v5.5 ===");
  Serial.println("NO BLUETOOTH VERSION");
  
  Serial.println("[1/6] TFT init...");
  tft.init();
  tft.setRotation(1);
  ...
  tft.setTextSize(2);
  tft.setCursor(10, 10);
  tft.println("NetRadio v5.5");
  tft.setTextSize(1);
  tft.setCursor(10, 35);
  tft.println("NO BLUETOOTH");
  Serial.println("[OK] TFT");
  
  Serial.println("[2/6] Touch init...");
  touch.begin(SPI);
  touch.setRotation(1);
  Serial.println("[OK] Touch");
  
  Serial.println("[3/6] SD Card init...");
  SPI.begin(18, 19, 23, SD_CS);
  sdCardDetected = SD.begin(SD_CS);
  Serial.println(sdCardDetected ? "[OK] SD Card" : "[FAIL] SD Card");
  
  Serial.println("[4/6] Audio init...");
  audio.setPinout(I2S_BCLK, I2S_LRC, I2S_DOUT);
  audio.setVolume(volume);
  Serial.println("[OK] Audio");
  
  Serial.println("[5/6] WiFi init...");
  tft.setCursor(10, 50);
  ...
  Serial.println("[6/6] Web server init...");
  server.on("/", []() {
    ...
    html += "</style></head><body><h1>NetRadio v5.5</h1>";
    ...
    html += "<button style='background:#F44336' onclick=\"fetch('/volup')\">Vol+</button>";
    html += "</body></html>";
  ...
  Serial.println("[PLAY] Starting audio...");
  ...
  Serial.println("NO BLUETOOTH in this version!");
}
