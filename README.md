# NetRadio v.3 - ESP32-2432S028 (CYD) Internet Radio

Интернет-радио на базе платы ESP32-2432S028 (Cheap Yellow Display) с встроенным TFT дисплеем 2.8" и SD картой.
...
### Тачскрин (встроенный):
Тачскрин XPT2046 уже встроен в дисплей и использует SoftSPI:
- TOUCH_CS: GPIO 33
- TOUCH_MOSI: GPIO 32
- TOUCH_MISO: GPIO 39
- TOUCH_CLK: GPIO 25

На экране отображаются 4 сенсорные кнопки:
- **PREV** (синяя) - предыдущая станция
- **NEXT** (зелёная) - следующая станция
- **VOL+** (оранжевая) - увеличить громкость
- **MODE** (фиолетовая) - сменить режим дисплея

### Кнопки (опционально, физические):
```
